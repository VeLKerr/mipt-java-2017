...
public class StringParser{
    Token currentToken = new Token('s');
    Token previousToken = new Token('s');
    private Stack<Double> numbers;
    private Stack<Character> operators;
    private int pos = 0;
    private double answer = 0;
    ...
    private Token getNextTocken() throws ParsingException{
        Token t = new Token('f');
        Character ch = sample.charAt(pos);
        if (ch <= '9' && ch >= '0') {
            if(wasSpace && currentToken.type == 1)
    ...
    public void Parse() throws ParsingException{
        ...
        operators.push('(');
        for(; pos < sample.length(); ++pos) {
            currentToken = getNextTocken();
            if (currentToken.equal('f'))
                continue;
            if(previousToken.equal('(') && (/*currentToken.equal('+')||*//* currentToken.equal('-'))){
                numbers.push(0.0);
            }
