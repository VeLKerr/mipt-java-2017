  ...
  private Operation getOperationTree(String expression) throws ParsingException {
    try {
      ...
      if (ind == 0) { // this is unary operation
        ...
        return new UnaryOperation(c,
          getOperationTree(expression.substring(1, expression.length())));
      } else { // this is binary operation
        String leftExpression = expression.substring(0, ind);
        String rightExpression = expression.substring(ind + 1, expression.length());
        return new BinaryOperation(getOperationTree(leftExpression), c,
          getOperationTree(rightExpression));
      }
    }
  }
